...
public class ControllerPersonaje : MonoBehaviour
{
    ...
    void MoverPersonaje()
    {
        ...
        if (grounded)
        {
            ...
            if (pInput.inputHorizontal != 0)
            {
                ...
                {
                    if (Mathf.Sign(pInput.inputHorizontal) != Mathf.Sign(rb.velocity.x) && Mathf.Abs(rb.velocity.x) > 3)
                    {
                        if (tengoMaxspeed)
                        {
                            print("cambiosentido2");
                            Vector2 direccion = Vector2.Perpendicular(normal).normalized * coefDeceleracion * -pInput.inputHorizontal;
                            this.rb.AddForce(direccion * fuerzaCambioSentido * 2 * Time.deltaTime);
                ...
                if (Mathf.Abs(rb.velocity.x) < (velMinima / 50))
                {
                    speed = velMinima;

                    Vector2 direccion = Vector2.Perpendicular(normal).normalized * speed * -pInput.inputHorizontal;
                ...
                if (Mathf.Abs(this.rb.velocity.x) < velMaxima / 50)
                {
                    //if (pInput.inputHorizontal > 0)

                    speed += coefAceleracion;
                    Vector2 direccion = Vector2.Perpendicular(normal).normalized * speed * -pInput.inputHorizontal;
        ...
        else
        {
            ...
            if (pInput.inputHorizontal != 0)
            {
                ...
                if (Mathf.Abs(rb.velocity.x) < (velMinima / 60))
                {
                    speed = velMinima;
                    Vector2 direccion = new Vector2(1, 0).normalized * speed * pInput.inputHorizontal;
                    //    this.rb.AddForce(direccion * 1.1f * Time.deltaTime);
                    //if (pInput.ultimoInputHorizontal == 1)
                    //{
        ...
        if (Mathf.Abs(rb.velocity.x) < velMaxima / 60)
        {
            tengoMaxspeed = false;
    ...
    void AplicarGravedad()
    {
        ...
        if (grounded)
        {
            ...
            rb.AddForce(-normal * 100 * Time.deltaTime);
            //}
            Collider2D[] hit = Physics2D.OverlapCircleAll(this.gameObject.transform.position, 1.1f);
            foreach (Collider2D col in hit)
            {
        ...
        else
        {
            ...
            looping = false;
            if (constantegravedad > 300) constantegravedad = 300;
            ...
            if (rb.velocity.y < -10)
            {
                ...
                constantegravedad *= (90 * Time.deltaTime);
            ...
            else if (rb.velocity.y < -5f)
            {
                ...
                /* if (rb.velocity.y < -12) rb.velocity = new Vector2(rb.velocity.x, -12f);*//*rb.AddForce(new Vector2(0, -0.005f)); *//*rb.velocity = new Vector2(rb.velocity.x,-5f);*/
                constantegravedad *= (85f * Time.deltaTime);
                rb.AddForce(new Vector2(0.0f, -constantegravedad * Time.deltaTime), ForceMode2D.Force);
            }
            else if (rb.velocity.y < 0f)
            {
                rb.AddForce(new Vector2(0, -400f * Time.deltaTime));
                /* if (rb.velocity.y < -12) rb.velocity = new Vector2(rb.velocity.x, -12f);*//*rb.AddForce(new Vector2(0, -0.005f)); *//*rb.velocity = new Vector2(rb.velocity.x,-5f);*/
                constantegravedad *= (80f * Time.deltaTime);
                rb.AddForce(new Vector2(0.0f, -constantegravedad * Time.deltaTime), ForceMode2D.Force);
            }
            ...
            else if (rb.velocity.y <= 15f)
            {
                if (ultimaNormal.y > 0)
                {
                    rb.AddForce(new Vector2(0, -400f * Time.deltaTime));
                    /* if (rb.velocity.y < -12) rb.velocity = new Vector2(rb.velocity.x, -12f);*//*rb.AddForce(new Vector2(0, -0.005f)); *//*rb.velocity = new Vector2(rb.velocity.x,-5f);*/
                    constantegravedad *= (70f * Time.deltaTime);
                    rb.AddForce(new Vector2(0.0f, -constantegravedad * Time.deltaTime), ForceMode2D.Force);
                }

            }

            rb.AddForce(new Vector2(0.0f, -250) * Time.deltaTime);
    ...
    void Dash()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (rb.velocity.x > 0)
                {
                    rb.velocity = new Vector2(velMaxima / 60, rb.velocity.y);
                }
                else
                {
                    ...
                    if (pInput.inputHorizontal != 0)
                    {
                        if (pInput.ultimoInputHorizontal > 0)
                        {
                            if (tengoMaxspeed == false)
                            {
                                ...
                                rb.velocity = new Vector2(velMaxima / 50 * 0.3f, rb.velocity.y);
                            }
                            else
                            {
                                rb.velocity = new Vector2(velMaxima / 50 * 0.9f, rb.velocity.y);
                            }
    ...
    void DetectarSuelo()
    {
        ...
        if (normal != null)
        {
            if (normal.y < 0.25 && normal.y > -0.25)
            {
