...
public class ControllerPersonaje : MonoBehaviour
{
    ...
    void Dash()
    {
        ...
        if (estoyDasheando == false)
        {
            ...
            if (joystick != null)
            {
                if ((joystick.Action3.WasPressed || Input.GetButtonDown("Dash")) && mEnergy.actualEnergy > mEnergy.energiaDash && (Mathf.Abs(joystick.LeftStick.X) >= 1.2f || Mathf.Abs(pInput.Yinput) < 1.8))
                {
                    ...
                    if (pulsadoEspacio == false)
                    {
                        if (pInput.inputHorizontal != 0)
                        {
                            ...
                            if (saltoInmediato == false)
                            {
                                ...
                                {
                                    auxTiempoTrasSaltoLoop = tiempoTrasSaltoLoop;
                                    if (normal.y < 0.8f && normal.y > -0.8f) rb.velocity = Vector2.zero;                                    //print("saltospeed" + rb.velocity);
                                }
                                else
                                {
                                    //rb.velocity = new Vector2(rb.velocity.x, ultimaNormal.y * fuerzaSaltoMin);
